...
// RevokeShareLinksFor closes every live link on the given notes. A rotation exists to take
// reading rights back, and a public link is a reading right that outlives the person who
// made it.
func (r *VaultRepository) RevokeShareLinksFor(
	ctx context.Context,
	fileIDs []int64,
	actorID int64,
) (int, error) {
	...
	err := inTx(ctx, r.pool, func(tx pgx.Tx) error {
		...
		rows, err := tx.Query(ctx, revoke, fileIDs)
		if err != nil {
			return fmt.Errorf("revoke share links: %w", err)
		}

		type closedLink struct{ vaultID, fileID int64 }

		var links []closedLink

		for rows.Next() {
			var link closedLink

			if err := rows.Scan(&link.vaultID, &link.fileID); err != nil {
				rows.Close()
				return fmt.Errorf("scan revoked link: %w", err)
			}
		...
		if err := rows.Err(); err != nil {
			return fmt.Errorf("iterate revoked links: %w", err)
		}

		for _, link := range links {
			err := recordAudit(ctx, tx, auditEntry{
